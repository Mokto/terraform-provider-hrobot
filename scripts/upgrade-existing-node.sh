...
Restart=always
RestartSec=2
ExecStart=/usr/bin/bash -c 'while true; do for i in {1..3}; do ip neigh replace 10.1.0.1 lladdr ${GATEWAY_MAC} dev ${VLAN_IFACE} nud reachable 2>/dev/null || true; sleep 0.2; done; sleep 5; done'
